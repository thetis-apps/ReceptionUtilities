...
exports.handle = async (event, x) => {
    ...
    let setup = dataDocument.ReceptionUtilities;
    
    if (setup.assignLocationToItem) {
    	
	    response = await ims.get('globalTradeItems/'+ detail.globalTradeItemId);
